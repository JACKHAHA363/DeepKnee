...
def inverse_pair_mapping(l, m, img_size, ps=128, smoothing=7):
    w, h = img_size
    pad_w = int(np.floor(w / 3.))
    pad_h = int(np.floor(h / 3.))
    ...
    hm[pad_w:pad_w + ps, :ps] = l
    hm[pad_h:pad_h + ps, (h - ps):] = m
    return hm
...
if __name__ == '__main__':
    ...
    for snp in config.snapshots:
        nets_snapshots_names.extend(glob(os.path.join(config.path_folds, snp, '*.pth')))

    net = KneeNetEnsemble(nets_snapshots_names, mean_std_path=os.path.join(config.path_folds, 'mean_std.npy'))
    if torch.cuda.is_available():
        net.cuda()
    ...
    for path_test_file in tqdm(paths_test_files, total=len(paths_test_files)):
        img, heatmap, probs = net.predict(path_test_file, 16)
